...
const PanpaaduSection = ({ homeData }: Props) => {
  const panpaaduList: Magazine[] = homeData?.["பண்பாடு"] || [];

  if (!panpaaduList.length) return null;

  return (
    <section className="w-full lg:min-h-[80vh] bg-black py-10 flex justify-center">
      {panpaaduList.map((panpaaduBook) => (
        <div className="w-full max-w-6xl px-6" key={panpaaduBook.id}>
          {/* Heading */}
          <motion.h2
          ...
          <div className="flex items-end justify-end mb-10">
            <span className="text-sm text-white">
              மொத்தம் {panpaaduList.length} கட்டுரைகள்
            </span>
          </div>
          ...
          >
            ...
            >
              <div className="relative w-80 h-[400px]">
                <Image
                  src={panpaaduBook.coverImage}
                  alt={panpaaduBook.title}
                  fill
            ...
            <div>
              ...
              <p className="text-gray-300 mb-2 flex items-center gap-2">
                ...
                <span>சமூக ஆய்வாளர் {panpaaduBook.author}</span>
              </p>

              <p className="text-gray-400 leading-relaxed mb-6">
                {panpaaduBook.subTitle}
              </p>
...
export default PanpaaduSection;
